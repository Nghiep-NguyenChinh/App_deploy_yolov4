...
sys.path.append("Faster_RCNN/keras_frcnn")

from Faster_RCNN.keras_frcnn import roi_helpers
...
def predict(num_rois, C, img_name, class_mapping, class_to_color, model_rpn, model_classifier, bbox_threshold = 0.7):
    ...
    for key in bboxes:
        ...
        for jk in range(new_boxes.shape[0]):
            ...
            (retval,baseLine) = cv2.getTextSize(textLabel,cv2.FONT_HERSHEY_COMPLEX,0.7,1)
    ...
    time_pred = time.time() - st
    # cv2.imwrite('./results_imgs/{}.png'.format(idx),img)
    return img, time_pred
